function Final_Mode = FMD(fs, x, FilterSize, CutNum, ModeNum, MaxIterNum)
    ...
    freq_bound = 0:1 / CutNum:1 - 1 / CutNum;   % 生成一个从 0 开始，均匀分布在 $[0, 1)$ 范围内、包含 $\text{CutNum}$ 个点的归一化频率序列。
    ...
    for n = 1:length(freq_bound)   % 循环会遍历 freq_bound 中的每一个起始频率，并为每个频率段生成一个单独的滤波器
        ...
        temp_filters(:, n) = fir1(FilterSize - 1, [freq_bound(n) + eps, freq_bound(n) + 1 / CutNum - eps], w)';  % # 生成具有截止频率区间[fl,fu（起始频率 + 步长）]的个 FIR 滤波器
    end
...
function [y_final f_final, ckIter T_final] = xxc_mckd(fs, x, f_init, termIter, T, M, plotMode)
    ...
    while n == 1 || (n <= termIter)
        ...
        end                               % 选择yt矩阵中除了第m+1列以外的所有列

        % Calculate beta
        beta = prod(yt, 2);  %计算beta项,计算每行的乘积
