...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    Bi = cell(1, n); 
    for i=1:n
        % Compute correlation matrix W（类似于，在个样本点的邻域上进行PCA）
        Ii = ni(i,:);               %第一个点的邻域
        Ii = Ii(Ii ~= 0);
        kt = numel(Ii);             %produce size
        Xi = X(Ii,:) - repmat(mean(X(Ii,:), 1), [kt 1]);     %去均值
        W = Xi * Xi'; 
        W = (W + W') / 2;           %（类似）协方差矩阵——相关系数矩阵
    ...
    tol = 0;
	if strcmp(eig_impl, 'JDQR')       %Jacobi-Davidson QR分解
        options.Disp = 0;
    ...
    end                                                         % B为奇异矩阵（det(B)=0）时,特征值全为 0，无法进行特征值分解
